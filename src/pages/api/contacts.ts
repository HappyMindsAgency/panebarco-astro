...
export async function POST({ request }: { request: Request }) {
  let body: { formType: 'brief' | 'talent'; data: Record<string, any> };
  ...
  if (formType !== 'brief' && formType !== 'talent') {
  ...
  const errors: string[] = [];
  const gasUrl = formType === 'brief' ? GAS_URL_BRIEF : GAS_URL_TALENT;
  ...
  try {
    ...
    const { subject, text, html } =
      formType === 'brief' ? infoBrief(data) : infoTalent(data);
  ...
  try {
    const { subject, text, html } =
      formType === 'brief' ? infoBriefRedazione(data) : infoTalentRedazione(data);
